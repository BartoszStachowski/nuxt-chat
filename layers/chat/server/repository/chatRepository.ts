...
export const createMessageForChat = async (data: {
  content: string;
  role: 'user' | 'assistant';
  chatId: string;
...
export const deleteMessagesForChat = (chatId: string): void => {
  const chat = chats.find((c) => c.id === chatId);
  if (chat) {
    chat.messages = [];
...
export const getLastMessageForChat = (chatId: string): ChatMessage | null => {
  const chat = chats.find((c) => c.id === chatId);
